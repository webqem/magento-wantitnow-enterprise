...
class Webqem_Mailcall_Block_Adminhtml_System_Config_Form_Field_Dayandhours extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $html = '<input type="hidden" value="'.count($this->_getValue('allowday')).'" id="dayandhours_condition_num"><div id="dayandhours_condition_template" style="display:none">';
        ...
        $html .= '<ul id="dayandhours_condition_container">';
        ...
        $html .= '</ul>';
        $html .= $this->_getAddRowButtonHtml('dayandhours_condition_container',
            'dayandhours_condition_template', $this->__('Add Condition'));
    ...
    {
        $mailcallCarrierModel=Mage::getModel('webqemmailcall/carrier_mailcall');
    ...
    protected function _getAddRowButtonHtml($container, $template, $title='Add')
    {
        if (!isset($this->_addRowButtonHtml[$container])) {
            $this->_addRowButtonHtml[$container] = $this->getLayout()->createBlock('adminhtml/widget_button')
                    ...
                    //$this->__('Add')
                    ->setOnClick("Element.insert($('".$container."'), {bottom: $('".$template."').innerHTML.replace('[allowday_template][r0]','[allowday][r'+$('dayandhours_condition_num').value+']').replace('[allowhourfrom_template][r0]','[allowhourfrom][r'+$('dayandhours_condition_num').value+']').replace('[allowhourto_template][r0]','[allowhourto][r'+$('dayandhours_condition_num').value+']')});$('dayandhours_condition_num').value=($('dayandhours_condition_num').value*1)+1;")
    ...
    protected function _getRemoveRowButtonHtml($selector='li', $title='Remove')
    {
        if (!$this->_removeRowButtonHtml) {
            $this->_removeRowButtonHtml = $this->getLayout()->createBlock('adminhtml/widget_button')
                    ...
                    ->setLabel($this->__($title))
                    //$this->__('Remove')
                    ->setOnClick("Element.remove($(this).up('".$selector."'));if($('dayandhours_condition_num').value>0){ $('dayandhours_condition_num').value=$('dayandhours_condition_num').value-1;}")
                    ->setDisabled($this->_getDisabled())
                    ->toHtml();
        }
